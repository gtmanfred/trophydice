...
async def do_gold_roll(
    gold: int, headers: Dict = Depends(headers), db: Session = Depends(get_db)
):
...
async def do_ruin_roll(headers: Dict = Depends(headers), db: Session = Depends(get_db)):
...
async def do_combat_roll(
    dark: int, headers: Dict = Depends(headers), db: Session = Depends(get_db)
):
...
async def do_weak_roll(headers: Dict = Depends(headers), db: Session = Depends(get_db)):
...
async def do_help_roll(headers: Dict = Depends(headers), db: Session = Depends(get_db)):
    result = roll(light=1, dark=0)
    resp = Response(
...
async def do_hunt_roll(
    light: int, headers: Dict = Depends(headers), db: Session = Depends(get_db)
):
...
async def do_contest_roll(
    ...
    ruin = len(
        [
            ...
            if die.result == 1 and die.dice_type is DiceTypeEnum.dark
        ]
    )
    if ruin:
...
@router.get(
    "/reduction", response_model=Response, tags=["rolls"], operation_id="reduction"
)
async def do_reduction_roll(
    headers: Dict = Depends(headers), db: Session = Depends(get_db)
):
    result = roll(light=1, dark=0)
    resp = Response(
        message=(
